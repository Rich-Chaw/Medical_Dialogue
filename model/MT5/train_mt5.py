...
def setup_train_args():
    ...
    parser.add_argument('--model_config', default='config/model_config_dialogue_small.json', type=str, required=False,
                        help='选择模型参数')
    parser.add_argument('--vocab_path', default='vocabulary/vocab_small.txt', type=str, required=False, help='选择词库,UNUSED IN THIS CODE')
    parser.add_argument('--train_tokenized_path', default='data/train_tokenized.txt', type=str,
                        required=False,help='将原始训练语料tokenize之后的数据的存放位置,UNUSED IN THIS CODE')
    ...
    parser.add_argument('--dialogue_model_output_path', default='dialogue_model/', type=str, required=False,
                        help='对话模型输出路径')
    parser.add_argument('--pretrained_model', default='', type=str, required=False, help='预训练的MT5模型的路径')
    ...
    parser.add_argument('--train_path', default='../../data/train_human_annotation.txt', type=str, required=False, help='原始训练语料')
    parser.add_argument('--val_path', default='../../data/dev_human_annotation.txt', type=str, required=False, help='原始验证语料')
    parser.add_argument('--test_path', default='../../data/test_human_annotation.txt', type=str, required=False, help='原始测试语料')
...
def generate(model,tokenizer,test_list,args,device):
    ...
    for dialogue in test_list:
        ...
        print("inputs in generation:",inputs)
        ...
        for index in range(len(outputs)):
            ...
            print("dialogue_dict:",dialogue_dict)
...
def main():
    ...
    if args.model == 'train':
        ...
        if args.local_rank == 0:
            ...
            tb_writer.close()
            logger.info("generated with model " + checkpoint)
            if args.cl:
                model = MT5ForConditionalGeneration.from_pretrained(checkpoint)
            else:
                model = modeling_mt5_cl.MT5ForConditionalGeneration.from_pretrained(checkpoint)
    ...
    if args.model == 'test':
        ...
        if args.cl:
            ...
            model = modeling_mt5_cl.MT5ForConditionalGeneration.from_pretrained(min_model)
        model.to(device)
        generate(model, tokenizer, test_list1, args, device)
