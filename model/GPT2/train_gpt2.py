...
def repeatAction(str):
    """
    Judge whether duplicate action/intent slot values are generated
...
def setup_train_args():
    ...
    parser.add_argument('--model_config', default='../../common/config/model_config_dialogue_small.json', type=str, required=False,
                        help='Select model config file')
    parser.add_argument('--vocab_path', default='../../common/vocabulary/vocab.txt', type=str, required=False, help='Select vocabulary file')
    parser.add_argument('--train_raw_path', default='../../data/train_human_annotation.txt', type=str, required=False, help='Original training corpus')
    parser.add_argument('--dev_raw_path', default='../../data/dev_human_annotation.txt', type=str, required=False, help='Original dev corpus')
    parser.add_argument('--test_path', default='../../data/test_human_annotation.txt', type=str, required=False, help='Original test corpus')
    parser.add_argument('--train_tokenized_path', default='../../common/tokenizer/train_tokenized.txt', type=str,
                        required=False,
                        help='The storage location of the data after tokenizing the original training corpus')
    parser.add_argument('--dev_tokenized_path', default='../../common/tokenizer/dev_tokenized.txt', type=str,
                       required=False,
...
def generate(tokenizer, model, args):
    ...
    break_tokens = tokenizer.encode('<|endoftext|>')[1:-1] # [1:-1]是为了去除encode产生的开头标记
    ...
    for data_id, data in enumerate(datas[0:-1]):
        ...
        for turn_id, turn in enumerate(data.split('\n')):
            ...
            knowledge = turn.split('<|endofintent|>')[1].split('<|action|>')[0] # include <|knowledge|>  <|endofknowledge|>
            ...
            # get generated intent/action/response
            with torch.no_grad():
                if args.inference_type == 'predicted':
                    ...
                    while predicted_index not in break_tokens:
                        ...
                        # sometime model generate repeated actions/intents, we just use truncate actions/intents if this happens
                ...
                if args.inference_type == 'groundtruth':
                    ...
                    print("break_tokens",break_tokens)
                    while predicted_index not in break_tokens:
                        # finish predicting after getting <|endoftext|>
                        outputs = model(tokens_tensor)
                        predictions = outputs[0]
                        predicted_index = torch.argmax(predictions[0, -1, :]).item()
                        indexed_tokens += [predicted_index]
                        ...
                        print("tokens_tensor(inputs).shape:",tokens_tensor.shape)
                        ...
                        if '<|action|>' in predicted_text and (not get_action) and repeatAction(predicted_text.split('<|action|>')[1]):
                            get_action = True
                        ...
                        print("predicted_text after judge <intent/endof intent>:",predicted_text)
                        tokens_tensor = torch.tensor([indexed_tokens]).to('cuda')
                        if len(indexed_tokens) > MAX_LEN:
                            break
                    ...
                    if '<|response|>' in predicted_text and '<|endofresponse|>' in predicted_text:
                        ...
                        generated_responses.append(' ')
                    print("\npredicted_text in {} turn:".format(turn_id), predicted_text)
                    print("generated_intents:",generated_intents)
        ...
        print("\ngenerated_dict in {} data:{}".format(data_id, generated_dict[data_id]))
    with open('{}.json'.format(args.inference_result), 'wt', encoding='utf-8') as f:
        json.dump(generated_dict, f, indent=1,ensure_ascii=False)
...
def main():
    ...
    # Setup tokenizer
    print(type(args.vocab_path))
    tokenizer = BertTokenizer(vocab_file=args.vocab_path)
    tokenizer.add_special_tokens({'bos_token': '<|endoftext|>'})    # 序列开始标记
    tokenizer.add_special_tokens({'eos_token': '<|endoftext|>'})    # 序列结束标记
    ...
    vocab_size = len(tokenizer) #vocab_size = 13315
    ...
    # Setup model
    model, n_ctx = create_model(args, vocab_size) # n_ctx是截断长度，即每n_ctx裁一个样本
    model.to(device)
    ...
    if args.local_rank == 0:
        ...
        tb_writer, multi_gpu= init(args) # get tensorboard writer and if there are multi_gpus
    ...
    test_list = []
    if args.ft2:
        # ft2: use data_train as train_list , data_dev as test_list
        ...
        # divide data_train into train_list and test_list 
        data_list = []
        for x in data_train.split('\n')[0:-1]:
            if len(x.split(' ')) < 800:
                data_list.append(x)
    ...
    # start train
    train(model, device, train_list, multi_gpu, args, tokenizer, tb_writer, overall_step=0)
    ...
    if args.local_rank == 0:
        ...
        if args.eval_all_checkpoints:
            ...
            for x in range(1, args.epochs + 1):
                ...
                model.resize_token_embeddings(vocab_size)
                model.to(device)
                result = evaluate(model, device, test_list, multi_gpu, args, tokenizer, tb_writer, overall_step=0)
                if result < min_loss:
                    min_loss = result
...
def simple_generate_test():
    ...
    tokenizer.add_special_tokens(
        {'additional_special_tokens': ['<|context|>', '<|endofcontext|>', '<|user|>', '<|system|>',
            ...
            , 'degree', 'frequency', 'dose', 'check_item', 'medicine_category', 'medical_place',
                                       'disease_history']})
    best_model = '../../common/model/gpt2_test_ft2_ck_model/model_epoch1/'
    logger.info("starting generating")
    generate(tokenizer, best_model, args)
