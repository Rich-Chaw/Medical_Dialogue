...
class ArgsParser(object):
    def __init__(self):
        ...
        parser.add_argument('--action_evaluation', required=False,
                            help='evaluation_path')
        parser.add_argument('--generate_evaluation', required=False,
                             help='evaluation_path')
        self.parser = parser
...
def intent_evaluation():
    ...
    hyp_intent_info = open('hyp_intent.txt', 'w', encoding='utf-8')
    ref_intent_info = open('ref_intent.txt', 'w', encoding='utf-8')
    hyp_intent_list=[]
    ref_intent_list=[]
    hyp_value_list=[]
    ref_value_list=[]
    ...
    for x in result.keys():
        for y in range(min(len(result[x]['generated_intent']), len(result[x]['target_intent']))):
            ...
            #Process the ground truth intents
            ref_intent_temp = result[x]['target_intent'][y]
            ref_intent_temp = re.sub('\'\'', '', ref_intent_temp)

            # generate value list for bleu
            ref_delete_english=re.sub('[\sa-zA-Z<>|_]','',ref_intent_temp)  # 去除所有字母和<>|,只保留中文
            ...
            # generate intent list for bleu
            ref_intent_list.append(split_chinese(re.sub('<\|continue\|>','',ref_intent_temp)))

            # generate y
            ref_intent_temp = ref_intent_temp.split('<|continue|>')
            ref_intent_info.write(''.join(ref_intent_temp) + '\n')
            ...
            y_pred_intent.append(y_pred_intent_temp)            #（sample_num,intent_num）
            y_gt_intent.append(y_gt_intent_temp)
            y_pred_intent_slot.append(y_pred_intent_slot_temp)  #（sample_num,intent_slot_num）
            y_gt_intent_slot.append(y_gt_intent_slot_temp)

    ref_intent_info.close()
    hyp_intent_info.close()
